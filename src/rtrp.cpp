...
#define SPLIT_AND_ASSERT_SIZE_KVP1(var, resp, type) \
...
namespace rtrp
{
	impl::Result<responses::LevelResponse> RtResponseParser::parseLevelResponse(const std::string& resp)
	{
		...
		{
			auto levelObject = SPLIT_AND_ASSERT_SIZE_KVP2(levelObject, levelObjectString, objects::LevelObject);
	...
	impl::Result<responses::ListResponse> RtResponseParser::parseListResponse(const std::string& resp)
	{
		...
		{
			auto listObject = SPLIT_AND_ASSERT_SIZE_KVP1(listObject, listString, objects::ListObject);
	...
	impl::Result<responses::UserResponse> RtResponseParser::parseUserResponse(const std::string& resp)
	{
		...
		for (const auto& userString : userObjectsStrings)
			...
			{
				auto userObject = SPLIT_AND_ASSERT_SIZE_KVP4(userObject, userString, objects::UserObject);
